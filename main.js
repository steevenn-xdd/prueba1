    ...
    if (pairingCode && !conn.authState.creds.registered) {
        ...
        if (!!phoneNumber) {
            ...
            if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
                console.log(chalk.bgBlack(chalk.redBright("Comience con el código de país de su número de WhatsApp, Ejemplo: +18299416234")))
                process.exit(0)
            }
        } else {
            numeroTelefono = await question(chalk.bgBlack(chalk.greenBright(`Por favor, escriba su número de WhatsApp 😍\nPor ejemplo: +18299416234 : `)))
...
let prekey = []
let directorio = readdirSync("./MonkeyAuth")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./MonkeyAuth/${files}`)
})
} 
...
const directories = ['./MonkeyAuth/', './jadibts/']
...
setInterval(async () => {
  ...
  const bio = `🤖 ᴛɪᴇᴍᴘᴏ ᴀᴄᴛɪᴠᴏ: ${uptime} ┃ Team Monkey Cab - BOT ┃ Únete a nuestro equipo: https://chat.whatsapp.com/Li43LrNSm5e7060DTsgOei`;

...
—◉ *Contacto para apelaciones:* wa.me/18299416234
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
      ...
      case 'remove':
        if (chat.welcome && !chat?.isBanned) {
          ...
          for (const user of participants) {
            const pp = './src/perfiluber.jpg'; // Establecer pp como la ruta fija
            try {
              // No se necesita más el bloque try-catch ya que pp ya está definido
            } finally {
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: 'ᴛʜᴇ ᴍʏsᴛɪᴄ - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'https://github.com/Monkey-Cab-bot/Monkey-Cab'}}}}, aa);
